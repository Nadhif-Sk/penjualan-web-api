    ...
    <div class="col-md-12">
        <div class="box">
            <div class="box-header with-border">
                ...
                @else
                    <a class="btn btn-success" href="{{ route('produk.create') }}"><span
            ...
            <div class="box-body">
                @if(Request::get('keyword'))
                    <div class="alert alert-success alert-block">
                        Hasil Pencarian Produk dengan Keyword :
                        ...
                        @foreach($produk as $row)
                            <tr>
                                ...
                                <td><img class="img-thumbnail" width="100px"
                                        src="{{ asset('uploads/'.$row->gambar_produk) }}" />
                                </td>
                                <td>{{ $row->stok }}</td>
                                <td>
                                    <form method="post"
                                        action="{{ route('produk.destroy',[$row->kd_produk]) }}"
                                        onsubmit="return confirm('Apakah anda yakin akan menghapus data ini ?')">
                                        @csrf
                                        {{ method_field('DELETE') }}
                                        <a class="btn btn-warning"
                                            href="{{ route('produk.edit',[$row->kd_produk]) }}">Edit</a>
